...
exports.editEndpoint = async (ctx, next) => {
    ...
    let index = fileJson[type].endpoints.findIndex(x => x.name == name)
    if (index >= 0) {
        if (body.mockConfig) {
            fileJson[type].endpoints[index] = body.mockConfig
            await promisify(fs.writeFile)(configPath, JSON.stringify(fileJson, null, 2))
        }

        let endPoint = fileJson[type].endpoints[index]
        ...
        if (fileJson[type].endpoints[i].name == name) {
            ...
            if (endPoint.filePath) {
                ...
                try {
                    ...
                    console.error('unlink file failed.')
                    console.error(e)
                }
            }
